...
def classify(X):
    bernoullinb = pickle.load(open("SVM.sav", 'rb'))
...
if __name__ == '__main__':
    ...
    if answer == 1:
        review_text_1=review_text.lower()
        review_text_1=review_text.split("\\s+")
        if "good" in review_text or "better"in review_text or "best" in review_text:
            print ("It is a True and good review")
        else:
            print ("It is a True and bad Review")

    else:
        review_text=review_text.split("\\s+")
        if "good" in review_text or "better" in review_text or "best" in review_text:
